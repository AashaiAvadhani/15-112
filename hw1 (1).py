...
def codeDescriptionAnswer():
    return "s is a lowercase character in the alphabet and not a vowel"
...
def getTheCents(n):
    ...
    if(isinstance(n,int)):
        ...
        three_digit = int((n * 1000) % 1000)
        #one digit case
        if(three_digit % 100 == 0):
            decimal = (three_digit // 100)
            return decimal * 10
        #two digit case
        if(three_digit % 10 == 0):
            decimal = (three_digit) // 10
            print(decimal)
            return decimal
        #three digit case
        if(three_digit % 100 > 10 and three_digit % 100 < 100):
            decimal = (three_digit // 10)
            return decimal + 1
    else:
        return None

#ask about how I am supposed to return the answer
def playGuessingGame():
    #first line
    print("Let's play a guessing game! Think of a type of pet.")
    fur_question = input("Does it have fur?")
    #yes fur
    if(fur_question == "Yes"):
        fetch_question = input("Can you teach it to play fetch?")
        if(fetch_question == "Yes"):
            print("It's a dog!")
        else:
            print("It's a cat!")
        ...
        swim_question = input("Can it swim?")
        if(swim_question == "Yes" or swim_question == "YES"):
            print("It's a fish!")
        #cant swim
        else:
            print("It's a bird!")
...
def lineIntersection(m1, b1, m2, b2):
    ...
    #if the lines are parallel then the slopes are equivalent
    if(m1==m2):
        return None
...
#should be done CHECK FOR MORE EXTREMA CASES
def getKthDigit(n, k):
    n = abs(n)
    digit = (n // (10 ** k)) % 10
    return digit
...
def setKthDigit(n, k, d=0):
    ...
    #if the number is negative need to preserve the negative
    if(n < 0):
        n = abs(n)
        added_number = d * (10 ** k)
        eliminate_num = getKthDigit(n,k) * (10 ** k)
        orig_num_elimnated = n - eliminate_num
        answer = orig_num_elimnated + added_number
